...
### Still open (R1-2 remainder)

- R1-2 also asks whether the hard model's *advantage grows* in 3D or whether new constraint-resolution challenges arise. The rewrite covers contact handling (native 3D) but not this comparative-performance question — leave for the fuller discussion.

### Left for later (new content / analysis)

- **R2-m4** — define symbols / add schematic for D, L, G: new content.
- **R1-1** — add soft-model literature + discussion (7 specific refs): literature review + writing.
- **R2-M1, R2-M2, R2-M3, R1-3** — major concerns; substantive analysis / plots / discussion (see Sam's notes).
