...
Status markers in the `#` column: **✅** implemented (see Progress log for the exact action) · **⏳** partially addressed · no marker = open. All edits compile (25 pages).
...
- **R2-m4** (symbols / schematic for D, L, G) ✅ — Added **Fig. 3** (`fig:matrix_flow`) in §4.5: a dataflow schematic showing the single unknown γ (ℝ^C) → forces 𝓕 via D and stresses σ via L → velocities 𝓤 via M and growth rates ℓ̇ via the growth law → state update 𝓒^{k+1} via G and lengths ℓ^{k+1} → new separations Φ^{k+1}, with the transposes Dᵀ, Lᵀ closing the complementarity loop. Every box carries its dimension; caption gives the exact linearization. Also added a cross-reference sentence after the L definition. Renders cleanly on p.5.

- **R2-M1** (φ>5 intrinsic vs under-resolved) ✅ — **Derivation + measurement, with the production code.** Three-part answer:
  1. **Reframed the text** (§6.2 + conclusion): φ>1 is a finite-stiffness force-balance effect, not under-resolution.
  2. **Added a derivation** in §4.4.2 (Eqs. `eq:effective_stiffness`, `eq:stability_factor`, `eq:stability_limit`): from the overdamped dynamics (Eq. 1), linear-stability of explicit Euler gives the maximum stable step-size Δt < ζℓ/k_eff ∝ ζ/k_cc. Self-contained, no external citation (literature search found only the qualitative "stiffer → smaller Δt"; the exact "Δt<ζ/k_eff" is *not* from a paper — it's derived here).
  3. **Validated numerically** by building the C++ simulator locally (brew PETSc/MPI/libomp) and running two sweeps at λ=10⁻⁴. Result = new full-width **Fig. 5** (`fig:soft_convergence`, `soft_overlap_analysis.png`), 3 panels: **(a)** four step-sizes (10× CFL range) collapse onto one φ_center(R) curve up to R=50 → converged in Δt; **(b)** raising k_cc drives φ_center → rigid limit 0.9; **(c)** measured critical CFL factor scales as **k_cc^(−0.96)** across a 25× stiffness range, matching the derived k_cc^(−1) (16-run stability grid).
  - Earlier confound corrected: the adaptive Δt is velocity-limited (= c_CFL·ε/u_m), so I do **not** cite the raw Δt values as a threshold measurement; panel (c) measures the threshold cleanly via the largest stable CFL per k_cc.
  - Reproducible analysis + **notebook** (`plot_soft_convergence.ipynb`, repo plotting style) + CSVs in `analysis/m1_overlap_convergence/`. Local macOS build tweaks reverted (repo pristine; documented in that README). Paper compiles, **25 pages**.
- **R2-M2** (quantitative measures) ✅ — Two parts. (1) **Rephrased** §6.1/§6.3 to present the metrics we already had as an explicit quantitative comparison: radial density (`fig:radial_distribution_packing_fraction`), ring spacing (ξ≈25, §7.7), colony expansion rate (slope of `fig:sim_time_vs_colony_radius`), microdomain size distribution (`fig:cluster_area_boxplot`). (2) **Added the orientational correlation length** — the one genuinely new metric — computed locally with the production code: ran hard+soft at λ∈{10⁻²,10⁻³,10⁻⁴} to R≈48, computed the nematic correlation function C(r)=⟨cos 2Δθ⟩ and correlation length ξ_θ. New **Fig. 14** (`fig:orientation_correlation`) + paragraph in §6.5. Result: ξ_θ grows with λ (≈1.2→1.9 cell lengths) and is **comparable between models**, showing the hard/soft difference is large-scale (domain size, Fig. 13) not local order — sharpens the mechanism. Used symbol ξ_θ to avoid clash with ring-spacing ξ. Analysis/data/notebook in `analysis/m1_overlap_convergence/` (`orient_corr.py`, `orient_fig.py`, `orient_*.csv`). *Optional remaining:* a quantitative experimental-value table (our ξ / domain sizes vs `You2018`, `Weady2024`) — pure literature lookup, could add to rebuttal.

### Resolved

- **M1 convergence figure** — DONE (option A): installed PETSc/MPI/libomp via brew, built the production C++ simulator locally, ran the sweep, produced Fig. 5. See the R2-M1 entry above and `analysis/m1_overlap_convergence/`.
- **M2 orientation correlation** — still open; same local-build path is now available if wanted (simulator builds + runs on this machine), otherwise Sam's lane.
