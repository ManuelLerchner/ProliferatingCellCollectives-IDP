...
- **R2-M2** (quantitative measures) ⏳ — **Mostly rephrased (no data needed).** §6.1 now presents radial density (`fig:radial_distribution_packing_fraction`), ring spacing (ξ≈25, §7.7), colony expansion rate (slope of `fig:sim_time_vs_colony_radius`), and microdomain size distribution (`fig:cluster_area_boxplot`) as an explicit **quantitative** comparison; added an expansion-rate sentence in §6.3. *Remaining:* (a) **orientational correlation length** — the one genuinely new metric, needs snapshots (Sam's lane or a small local run); (b) a quantitative **experimental-value table** (our ξ / domain sizes vs `You2018`, `Weady2024`) — literature lookup.
