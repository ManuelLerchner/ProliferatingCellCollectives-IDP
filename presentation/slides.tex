...
\begin{frame}
    \frametitle{Hard Collision Model: Implementation}


    \begin{itemize}
        \item LCP reformulated as energy minimization:
              \begin{equation*}
                  \small
                  \min_{\boldsymbol{\gamma} \geq \mathbf{0}} E(\boldsymbol{\gamma}) = \boldsymbol{\gamma}^\top \boldsymbol{\Phi}^k + \frac{\Delta t}{2}\, \boldsymbol{\gamma}^\top \mathbfcal{D}^\top \mathbfcal{M}^k \mathbfcal{D}\, \boldsymbol{\gamma} + \mathbf{1}^\top \frac{\Delta t}{\lambda}
                  \left( \frac{\boldsymbol{\ell}}{\tau} \odot e^{-\lambda\, \mathbfcal{L}\, \boldsymbol{\gamma}} \right)
              \end{equation*}
        \item Key property: $\nabla E(\boldsymbol{\gamma}^*) = \boldsymbol{\Phi}^{k+1}(\boldsymbol{\gamma}^*)$
        \item Minimum satisfies all three constraints automatically!
        \item Solve via BBPGD (Barzilai-Borwein Projected Gradient Descent)
    \end{itemize}
    ...
    \begin{columns}
        \begin{column}{0.5\textwidth}
            ...
            \begin{itemize}
                ...
                \item \cmark \; 30$\times$ larger timesteps
                \item \cmark \; Accurate stress
            \end{itemize}
        \end{column}
        \begin{column}{0.5\textwidth}
            \textbf{Disadvantages:}
            \begin{itemize}
                \item \xmark \; Global solver
                \item \xmark \; High per-step cost
