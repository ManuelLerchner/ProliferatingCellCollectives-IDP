...
To our knowledge, a direct, systematic comparison of these models remains limited in the literature. Existing biological studies often validate their approaches against experimental data, such as microscopy images, growth curves, and morphological features~\cite{Rudge2012,Weady2024,Blanchard2015,Ghosh2015,You2018,Warren2019,Khan_2024,You_2021,Valdez2025,Rudge2013,Langeslay_2023}, with a primary emphasis on biological fidelity. Comparatively fewer works examine computational aspects such as runtime, scalability, and numerical stability across varying colony sizes, leaving limited guidance on selecting an appropriate modeling paradigm for a given problem scale.
...
Each spherocylinder is characterized by a time-varying length $\ell_i(t)$ that increases during growth prior to division, and a fixed diameter $d$ that remains constant across the population. This geometry enables efficient contact detection algorithms~\cite{GeometricTools} and naturally accommodates rotational dynamics essential for capturing realistic colony mechanics. Although bacteria are fundamentally three-dimensional objects, the simulations in this work restrict cells to a 2D plane, consistent with early colony growth on a flat substrate. All quantities (forces, torques, orientations) are three-dimensional vectors; confining them to a plane is a standard simplification, and the extension to full 3D is straightforward.
...
To account for Brownian motion and other sources of random fluctuations, we apply a small random rotational perturbation to each cell at every timestep. This is particularly important in the early colony phase, where few mechanical interactions exist to break the orientational symmetry: without noise, cells that happen to be aligned would remain so indefinitely, producing artificially ordered initial configurations that do not reflect biological reality. The perturbation models the tendency of bacteria to explore their local environment and reorient as they spread across a flat substrate. As the colony densifies, mechanical interactions take over as the dominant driver of orientational dynamics, and the diffusion becomes negligible by comparison. The magnitude of the perturbation was determined empirically to produce realistic spreading behavior in very small colonies of only a few cells, while remaining small enough to not artificially disorder larger, mechanically-driven colonies.
...
Both models share the same colony representation, growth law, stress computation, and timestepping infrastructure; only how contact force magnitudes are determined differs. In both cases, forces act along the same contact normal: the hard model derives magnitudes from Lagrange multipliers enforcing strict non-overlap, while the soft model uses a local repulsive potential. All downstream mechanics respond identically, ensuring that any differences in physical behavior or performance are attributable solely to the collision model. This controlled setup enables a direct, systematic comparison that has been absent from prior work, where hard and soft models were typically developed and evaluated in isolation.
...
The distinction between hard and soft collision models lies solely in how the generalized force vector $\mathbfcal{F}$ and in particular the inter-cell forces $\mathbf{F}_{ij}$ are computed. Specifically, the hard model computes $\mathbf{F}_{ij}$ via Lagrange multipliers from constraint solving (\autoref{sec:hard_model}), while the soft model uses repulsive potentials (\autoref{sec:soft_model}). The rest of the framework, including state representation, dynamics, and integration, remains identical.
...
At the optimal solution $\boldsymbol{\gamma}^*$, the Karush-Kuhn-Tucker (KKT) conditions hold. These are the standard first-order optimality conditions for constrained optimization: they generalize the unconstrained condition $\nabla E = 0$ to account for inequality constraints, and are both necessary and sufficient here due to convexity~\cite{Nocedal2006}. Remarkably, they map directly onto our physical constraints:
...
We solve the constrained optimization problem using the Barzilai-Borwein projected gradient descent method (BBPGD)~\cite{BBPGD}, following the implementation described in~\cite{Weady2024SM,Yan2019}. BBPGD is an efficient iterative solver for finding the minimum of a constrained energy function: at each iteration it takes a gradient descent step to reduce $E(\boldsymbol{\gamma})$, then projects the result onto the feasible region $\boldsymbol{\gamma} \geq \mathbf{0}$ to maintain repulsivity. The step size is selected adaptively using the Barzilai-Borwein rule, which estimates a quasi-Newton step from successive gradient differences, avoiding expensive line searches while achieving fast convergence for the box-constrained quadratic structure of the NCP. The solver iterates until convergence, yielding the optimal contact forces $\boldsymbol{\gamma}^*$, which are then used to compute $\mathbfcal{F}(\boldsymbol{\gamma}^*)$ and $\dot{\boldsymbol{\ell}}(\boldsymbol{\gamma}^*)$ for updating the colony state via \autoref{eq:colony_update}.
...
To address this limitation, Weady et al.~\cite{Weady2024SM} developed the recursively generated linear complementarity problem (ReLCP) method. After each BBPGD convergence, ReLCP identifies any remaining overlaps by examining all pairwise cell distances, generates new constraints for these violations, and re-solves the complementarity problem with the augmented constraint set. This process iterates until all cell overlaps fall below the tolerance $\epsilon$. This hierarchical approach ensures that the non-overlap condition is strictly enforced, even in the presence of complex contact geometries and coupled motions (see \autoref{fig:bbpgd_residual}). In practice, convergence within $\leq 6$ ReLCP iterations is consistently observed (see \autoref{fig:bbpgd_energy}), making it computationally tractable for most simulations~\cite{Weady2024SM}, though no formal convergence bound is available in the literature.
...
Collisions are processed through a unified pipeline designed to decouple geometry from physics. The process consists of three phases: (1) broad-phase detection using a spatial grid (reducing neighbor searches to $O(N)$), (2) narrow-phase detection that computes exact contact points, normals, and lever arms, and (3) force and stress assembly, in which the collision model computes per-contact forces and torques that are accumulated into the global force vector and used to update the colony stress state. This structure is passed to either the hard or soft collision model, which computes the resulting forces and torques. This abstraction allows seamless switching between collision models without altering the overall simulation flow.

Each collision detector call generates a single constraint per neighboring cell-pair within a threshold distance $d = 0.5$, equal to one cell diameter, ensuring both overlapping cells and cells in near-contact are captured for constraint generation. As this threshold is larger than zero, it captures both overlapping and closely adjacent cells. This overapproximation is crucial for the stability of the hard model. For the soft model, those non-overlapping cell-pairs contribute no forces (since $\delta < 0$), while for the hard model they provide additional global information that improves constraint resolution. Ideally, constraints would be constructed between all cell pairs regardless of distance, but for computational efficiency only nearby cells are considered~\cite{Yan2019, Yan2022,Yan_2020}.
...
where $c = 0.5$ is the CFL safety factor (ensuring cells move at most half the tolerance per step), $\varepsilon = 10^{-3}$ is the solver displacement tolerance, and $u_m$ is the median particle speed including growth as defined above. Choosing $\Delta t$ this way ensures that most cells move less than half the solver tolerance in each timestep, preventing excessive overlaps that could destabilize the simulation.
...
The EMA smoothing in step 4 prevents oscillatory $\Delta t$ sequences that could destabilise the solver when velocity fluctuations are large. The 20\% clamp in step 5 limits single-step acceleration so that $\Delta t$ cannot jump abruptly to a value that would cause excessive overlaps before the next CFL check. Both the smoothing factor $\alpha = 0.01$ and the 20\% cap were determined empirically and found to work well across all tested colony sizes and stress sensitivities.
...
With both models sharing identical growth, stress, and timestepping infrastructure, we now compare their emergent behavior across two regimes: pattern formation driven by stress-sensitive growth, and colony-scale mechanical properties as the system grows large. We assess whether the choice of collision model affects qualitative or quantitative outcomes, and identify the conditions under which the two approaches converge.
...
A contributing factor to this excessive overlap is the adaptive timestep method (see \autoref{alg:adaptive_dt}), which bases $\Delta t$ solely on cell velocities but not on current overlap. Whilst a smaller $\Delta t$ would reduce overlap, incorporating instantaneous overlap into the timestep criterion would add per-step cost. We identify this as a direction for future improvement (see \autoref{sec:future_soft_model}).
...
\begin{figure}[H]
    ...
    \begin{subfigure}[b]{0.49\columnwidth}
        ...
        \caption{Soft model}
    \end{subfigure}
    \caption{Close-up comparison of cell packing in the colony center for both collision models. The hard model (a) maintains near-optimal packing, while the soft model (b) exhibits significant overlap and unphysical crowding.}
...
\begin{figure}[H]
    ...
    \begin{subfigure}[b]{\linewidth}
        ...
        \caption{Radial packing fraction profiles ($\phi = \frac{A_{\text{cells}}}{A_{\text{total}}}$) for both models, taken at colony radius $R = 100$. The x-axis shows distance from the colony center (non-dimensionalized, $\ell_0 = 1$). Here $A_{\text{cells}}$ is the total area occupied by cells within a radial bin of width 2, and $A_{\text{total}}$ is the annular area of that bin. The hard model maintains nearly optimal packing, while the soft model exhibits higher fractions in the colony center, especially at low stress-sensitivity ($\lambda = 10^{-4}$).}
    ...
    \begin{subfigure}[b]{\linewidth}
        ...
        \caption{Maximum cell overlap vs.\ colony radius (i.e.\ simulation progress). The hard model maintains overlap near the solver tolerance $\epsilon = 10^{-3}$, while the soft model exhibits a steady increase, reaching near-complete overlap by the simulation end. Overlap decreases radially because peripheral cells have more room to expand outward, while interior cells are mechanically compressed by accumulated colony pressure.}
...
\begin{figure}[H]
    ...
    \includegraphics[width=\linewidth]{figures/comparison_plots/analytical_radial_growth_rate.png}
    \caption{Radial relative growth rate profiles ($e^{-\lambda \sigma}$). \textbf{Top:} Simulated profiles for both models. Cells in the soft model grow significantly faster than those in the hard model across most of the colony, particularly at low stress-sensitivity ($\lambda = 10^{-3}$ and $\lambda = 10^{-4}$). At high stress-sensitivity ($\lambda = 10^{-2}$), growth is strongly suppressed in the interior of both models, with only cells at the colony edge actively dividing. \textbf{Bottom:} Analytical prediction $e^{-\lambda\bar{\sigma}(r)}$ from Weady et al.~\cite{Weady2024SM} for $R=100$. The simulation profiles qualitatively match the analytical trend (growth suppressed in the interior, active at the periphery), though the soft model overestimates interior growth due to its lower stress transmission.}
...
We find that the hard model consistently outperforms the soft model across all core counts ranging from 1.20 at 1 core to 9.36 at 112 cores, see \autoref{fig:runtime_hard_soft}. This performance advantage likely stems from the hard model's significantly larger stable timesteps, which allow more computational work per communication cycle. As shown in \autoref{fig:speedup_hard_soft}, the hard model achieves reasonable speedup up to approximately 64 cores, while the soft model scales well only up to about 32 cores.

The decrease in performance at higher ranks arises from the domain decomposition. At very high core counts, each rank handles only a narrow angular slice of the colony (see \autoref{fig:domain_decomposition}), so communication increasingly dominates runtime. With less local work per rank, ghost particle exchanges and global PETSc matrix operations (in the hard model), including matrix-vector products, vector additions, and scatter/gather operations, become more frequent and costly, especially when small timesteps $\Delta t$ are used. Despite this, the angular decomposition ensures each rank handles a geometrically similar colony slice, limiting load imbalance to a modest factor at the core counts tested. Nevertheless, the communication overhead at very high core counts reduces scaling efficiency and leads to longer runtimes.
...
In contrast, the hard model maintains a significantly larger critical timestep than that of the soft model. This allows each rank to advance particles substantially between communication steps, effectively amortizing the cost of MPI exchanges over more computational work. Consequently, the hard model sustains a favorable computation-to-communication ratio, explaining its better scaling at moderate to high core counts.
...
\begin{figure}[H]
    ...
    \caption{Adaptive timestep $\Delta t$ for both collision models over simulation time. In the early growth-dominated phase, both models share a similar $\Delta t$ set by cell growth velocities. As the colony densifies, the soft model enters a collision-dominated regime where large overlaps drive velocities up, forcing $\Delta t$ down to $\approx 10^{-5}$. The hard model avoids this: its constraint-based resolution keeps cells separated, so $\Delta t$ remains stable or even increases as interior growth decelerates.}
...
The average number of solver iterations per timestep, also plotted in \autoref{fig:cfl_vs_runtime}, increases with the CFL factor at low values, following $\text{iterations} \approx 1800 \cdot \text{CFL}$. At the optimal CFL, around 3600 iterations are required per physical step, after which BBPGD iteration counts rise sharply.
...
\begin{figure}[h]
    ...
    \caption{Total runtime to reach a colony radius of 100 versus CFL factor on 112 cores; each point is an independent simulation run. The U-shaped curve arises because small CFL values require many timesteps while large values inflate BBPGD iterations per step. The annotated $\Delta t$ is the mean timestep size at each CFL factor. The red region marks CFL values at which the BBPGD solver fails to converge.}
...
\begin{figure}[H]
    ...
    \begin{subfigure}[b]{\linewidth}
        ...
        \caption{Number of BBPGD iterations per timestep as a function of particle count for a representative run at $R \approx 100$ ($N \leq 175{,}000$). Iterations remain roughly $\mathcal{O}(1000)$ on average, with occasional spikes up to $\mathcal{O}(5000)$.}
...
Finally, \autoref{fig:huge_combined_stress_shared} shows the corresponding radial stress distribution. In contrast to the results in \autoref{fig:radial_distribution_stress}, the simulated stress profile closely follows the analytical prediction of Weady et al.~\cite{Weady2024SM}. At $R = 260$ with $\approx\!301{,}000$ cells, the colony satisfies the continuum limit assumptions underlying the analytical derivation, explaining the improved agreement relative to smaller colonies where discretization noise is significant.
...
\begin{figure}[H]
    ...
    \begin{subfigure}[b]{\linewidth}
        ...
        \caption{BBPGD iterations per timestep as a function of particle count for the hard model at maximum scale ($R = 260$, $N \leq 301{,}116$). Unlike \autoref{fig:bbpgd_iterations_per_step_vs_num_particles} which shows $R \approx 100$ behaviour, this shows the large-scale regime: each timestep resolves about 5{,}131{,}551 constraints over 8 ReLCP phases, requiring on average 7{,}800 iterations, with occasional peaks up to 25{,}000.}
