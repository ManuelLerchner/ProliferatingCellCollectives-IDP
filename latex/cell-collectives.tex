...
\begin{itemize}
    ...
    \item \textbf{Benchmarking}: Python scripts for strong-scaling studies, CFL factor sweeps, and colony-size runtime profiling.

    \item \textbf{Utilities}: Modules for parsing VTK output, loading multi-run datasets, generating simulation configurations, and rendering colony snapshots via ParaView state files.
\end{itemize}
...
A contributing factor to this excessive overlap is the adaptive timestep method (see \autoref{alg:adaptive_dt}), which bases $\Delta t$ solely on cell velocities but not on current overlap; a more sophisticated scheme could account for maximum overlap, though smaller step-sizes would reduce performance.
