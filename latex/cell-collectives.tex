...
The computational consequences are substantial: the soft model requires simulating
far more cells to reach the same colony size, directly increasing both memory usage
and runtime. However, these additional cells mostly represent unphysical overcrowding in the colony interior rather than realistic expansion.
...
We find that the hard model consistently outperforms the soft model across all core counts, being faster by a median factor of 1.83 (ranging from 1.20 at 1 core to 9.36 at 112 cores, see \autoref{fig:runtime_hard_soft}). This performance advantage likely stems from the hard model's significantly larger stable timesteps, which allow more computational work per communication cycle.

As shown in \autoref{fig:speedup_hard_soft}, the hard model achieves reasonable speedup up to approximately 64 cores, after which scaling efficiency degrades. In contrast, the soft model exhibits poor scaling: beyond 16 cores, increasing the core count actually increases runtime.

At very high core counts, each rank controls only a very small angular slice of the colony (see \autoref{fig:domain_decomposition}), and communication overhead begins to dominate. As the number of cores increases, the workload per core decreases, leading to more frequent ghost particle exchanges and neighbor list reconstructions relative to useful computation.
...
\begin{figure}[H]
    ...
    \begin{subfigure}[b]{1\linewidth}
        ...
        \caption{Speedup versus CPU cores. The hard model scales reasonably up to 64 cores, while the soft model shows poor scaling beyond 16 cores.}
...
A major factor contributing to the soft model's poor parallel performance is its very small critical timestep determined by \autoref{eq:cfl_dt}. As the colony grows, $\Delta t$ must be drastically reduced to maintain stable simulations (see \autoref{fig:simulation_time_vs_dt}). With many tiny timesteps, each rank performs only minimal computational work before initiating costly MPI communication, including ghost particle exchange and neighbor list reconstruction. As a result, communication and neighbor list updates dominate the total simulation time, which leads to poor scaling at high core counts.

In contrast, the hard model maintains a mostly constant timestep that is significantly larger than that of the soft model, allowing each rank to advance particle positions substantially between communications, effectively amortizing the cost of MPI exchanges over more useful computation. As a result, the hard model maintains a favorable computation-to-communication ratio, which explains its better scaling at moderate to high core counts.

As shown in \autoref{fig:simulation_time_vs_dt}, the hard model stabilizes around $\Delta t_{\text{hard}} \approx 3 \cdot 10^{-4}$ h, while the soft model steadily decreases to $\Delta t_{\text{soft}} \approx 10^{-5}$ h. The ratio $\Delta t_{\text{hard}}/\Delta t_{\text{soft}} \approx 35$ indicates that the hard model can take roughly 35 times larger steps than the soft model in dense colonies, requiring significantly fewer steps to simulate the same physical time. Notably, for the hard model with $\lambda = 10^{-2}$, the critical timestep even increases over time as cell growth in the interior slows down (see \autoref{fig:radial_distribution_growth_rate}) and the overall median velocity $u_m$ decreases.

Using adaptive timestepping proves highly advantageous, offering both stability and efficiency by adjusting the timestep to the colony's current dynamics. A fixed timestep cannot achieve this balance: steps that are too small waste computational effort when interactions are sparse, while steps that are too large can cause numerical instability in dense regions. This adaptivity enables long-term simulations to be carried out efficiently without compromising accuracy or stability.
...
The efficiency of the BBPGD solver for the hard model depends critically on the timestep size, which determines both the number of solver iterations per timestep and the total number of timesteps required. This relationship produces a characteristic U-shaped runtime curve as a function of the CFL number, as shown in \autoref{fig:cfl_vs_runtime}.
...
The average number of solver iterations per timestep, also plotted in \autoref{fig:cfl_vs_runtime}, increases roughly linearly with the CFL factor at low values, following $\text{iterations} \approx 1800 \cdot \text{CFL}$. At the optimal CFL, around 3600 iterations are required per physical step, after which iteration counts rise sharply as the linearized constraints lose accuracy.

By keeping each timestep close to the optimal CFL, adaptive timestepping maximizes computational work per solver iteration and minimizes total runtime, particularly in dense colonies where small steps would otherwise be dominated by communication and solver overhead. Furthermore, choosing an appropriate CFL factor optimally balances the work done by the constraint solver to resolve overlaps against the number of timesteps, allowing the hard model to further increase both performance and scaling.

All benchmarks in the previous section used a conservative CFL factor of 0.5, indicating that runtimes could be improved even more by using a higher CFL value closer to the optimal point.
...
In this section, we investigate the largest colony sizes that can be simulated within a fixed computational budget of 24 hours on 112 CPU cores of the \texttt{CoolMUC-4} cluster. A stress sensitivity of $\lambda = 10^{-3}$ is used to promote the emergence of characteristic radial wave patterns. At these extreme scales, only the hard model is analyzed, as the soft model exhibits heavy overcrowding and excessive cell overlap, rendering its results physically meaningless.
...
The associated solver workload is summarized in \autoref{fig:huge_bbpgd_iterations_per_step_vs_num_particles}. Each timestep at this scale requires resolving about 5{,}131{,}551 contact constraints, distributed across 8 ReLCP phases and an average of 7{,}800 BBPGD iterations. The number of BBPGD iterations per timestep grows approximately linearly with the number of particles, following $\text{Num}_{\text{BBPGD}} \approx 0.0267N$, although two pronounced spikes of up to 25{,}000 iterations occur during the simulation.
...
We demonstrated that both models successfully reproduce the concentric ring patterns and microdomain formation observed in experimental bacterial colonies, confirming that these macroscopic phenomena are emergent properties of growth-mechanics feedback and are robust to the specific details of collision resolution. This robustness justifies the use of computationally simpler models for studies focused on colony-level pattern formation.

Beneath this macroscopic similarity lie critical microscopic differences. The hard model enforces strict non-overlap, maintaining a realistic packing fraction of about 0.9 and producing reliable stress distributions and microdomain structures. In contrast, the soft model allows significant, unphysical cell overlap, with packing fractions exceeding 5 in colony centers. This distorts microdomain morphology and produces elongated, unrealistic cell bundles, particularly in large colonies or under low stress-sensitivity conditions.

Our performance benchmarking revealed that the hard model consistently outperforms the soft model across all colony sizes, achieving up to 9.36$\times$ faster runtimes on 112 cores. This advantage stems from its ability to use timesteps approximately 30 times larger ($\Delta t_{\text{hard}} \approx 3 \times 10^{-4}$ h vs. $\Delta t_{\text{soft}} \approx 10^{-5}$ h), which amortizes communication costs over more computational work. The hard model also exhibits better parallel scaling, maintaining reasonable speedup up to 64 cores, while the soft model shows poor scaling beyond 16 cores. Within a fixed 24-hour computational budget on 112 cores, the hard model successfully simulated colonies up to $R \approx 260$ with approximately 301,116 cells while maintaining physical accuracy.

The introduction of an adaptive timestepping algorithm based on the Courant-Friedrichs-Lewy condition proved essential for maintaining numerical stability, dynamically selecting stable timesteps tailored to the current colony state. For the hard model, we identified an optimal CFL factor of approximately 2 that minimizes total runtime by balancing solver iterations per step against the number of required timesteps.

In summary, the hard model is strongly preferred for nearly all applications: it enforces realistic packing densities, produces accurate stress distributions, and offers superior computational performance. The soft model may serve as a preliminary tool for exploratory studies of very small colonies ($R \lesssim 50$) at high stress-sensitivity, where packing artifacts remain somewhat limited and only pattern formation is of interest. However, its severe physical limitations, including unphysical overcrowding, distorted stress distributions, and artificial cell bundles, make it unsuitable for analysis or large-scale simulations.
...
All high-resolution figures and supplementary videos are available online at \url{https://home.cit.tum.de/~ler/bacteria/figures/} or \url{https://github.com/ManuelLerchner/ProliferatingCellCollectives-IDP}.
