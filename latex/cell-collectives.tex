...
\author{
    \IEEEauthorblockN{Manuel Lerchner~\orcidlink{0000-0002-5167-465X}\thanks{Corresponding author: \href{mailto:manuel.lerchner@tum.de}{manuel.lerchner@tum.de}}}
    \IEEEauthorblockA{
...
Simulating dense, proliferating cell collectives, however, poses significant computational challenges: the large number of cells in realistic colonies, combined with the need to resolve collisions accurately across continuously changing configurations, creates a computational bottleneck. Moreover, different collision-handling approaches, such as rigid constraints versus soft repulsions, offer distinct trade-offs in physical fidelity, numerical stability, and computational cost. This work is directly motivated by these considerations and builds upon the pioneering computational framework developed by Weady et al.~\cite{Weady2024}, who demonstrated how to model bacterial colony growth while capturing stress-dependent feedback and pattern formation. We extend their work by systematically comparing two fundamentally different approaches to collision handling, examining their respective strengths and limitations in simulating large systems of proliferating cells. Hard models enforce strict non-overlap via constraint solving, enabling large stable timesteps at higher per-step cost, while soft models use local repulsive potentials that are cheap per step but demand very small timesteps for stability. Adaptive timestepping based on the CFL condition is essential for both: without it, rapidly changing colony configurations cause either divergence or extreme inefficiency. Both approaches are implemented within a single unified framework, enabling direct, controlled performance comparisons that have been absent from the literature.
...
where $\tau$ is the $1/e$ growth timescale (the characteristic division time under zero stress), $\ell_0$ is the reference cell length at birth, $\sigma_i$ is the compressive stress along cell $i$'s main axis, and $\lambda^{'}$ is a stress sensitivity parameter. As the compressive stress increases, the effective growth rate $\dot{\ell}_i$ decreases toward zero, leading to slower elongation. When $\sigma_i = 0$, the cell grows exponentially with a rate determined solely by $\tau$.
...
Similar to \cite{Weady2024SM}, we nondimensionalize lengths by $\ell_0$, time by $\tau$, and stress by $\tau / (\zeta \ell_0^2)$, where $\zeta$ is the drag coefficient and $d$ is the cell diameter. This leaves the dimensionless growth sensitivity $\lambda = (\tau / \zeta \ell_0^2)\lambda^{'}$ as the only free parameter. Therefore we set $\ell_0 = 1$, $d = 0.5$, $\tau = 1$, and $\zeta = 1$.
...
Subsections~\ref{sec:colony_rep}--\ref{sec:state_integration} follow Weady et al.~\cite{Weady2024,Weady2024SM}. The soft collision model (\autoref{sec:soft_model}) and adaptive timestepping (\autoref{sec:adaptive_dt}) are novel contributions of this work.

\subsection{Colony Representation}
\label{sec:colony_rep}
...
where $\hat{\mathbf{n}}_\alpha$ is the normal vector at the contact point. The multipliers $\boldsymbol{\gamma}$ are solved fresh each timestep and carry no history; unlike $\mathbfcal{F}^k$, they require no $k$ superscript. The total generalized force vector $\mathbfcal{F}$ for the colony depends linearly on the multipliers $\boldsymbol{\gamma} = [\dots, \gamma_\alpha, \dots]^\top \in \mathbb{R}^{C}$ and can be conveniently expressed in matrix form:
...
\begin{enumerate}
    ...
    \item \textbf{Cell Growth:} Additionally, cell growth affects separation according to $\dot{\mathbf{\Phi}^k}_{\text{growth}} = \frac{d \mathbf{\Phi}^k}{d \boldsymbol{\ell}} \frac{d \boldsymbol{\ell}}{dt} =-(\nabla_{\boldsymbol{\ell}} \mathbf{\Phi}^k) \dot{\boldsymbol{\ell}}$.\newline The negative sign arises because as cells elongate, they reduce the separation distance between them. This growth term captures how cell elongation reduces separation distances during the timestep and is already fully accounted for in the linearized $\mathbf{\Phi}^{k+1}$ without additional correction.
\end{enumerate}
...
Noting that $\nabla_{\boldsymbol{\gamma}} E \approx \mathbf{\Phi}^{k+1}(\boldsymbol{\gamma})$, minimizing $E$ is equivalent to driving the linearized separation distances to zero — precisely the no-overlap condition. Because $E(\boldsymbol{\gamma})$ is convex, the constrained optimization problem has a unique global minimizer over the feasible region $\boldsymbol{\gamma} \geq \mathbf{0}$. We compute this minimum using a projected gradient method, which iteratively takes steps in the direction of the negative gradient and projects the result onto the feasible region to ensure repulsive forces.
...
We solve the constrained optimization problem using the Barzilai-Borwein projected gradient descent method (BBPGD)~\cite{BBPGD}, following the implementation described in~\cite{Weady2024SM,Yan2019}. At each iteration, BBPGD computes a gradient step with Barzilai-Borwein adaptive step-size selection, then projects $\boldsymbol{\gamma}$ onto $\boldsymbol{\gamma} \geq \mathbf{0}$ to enforce repulsivity. This avoids expensive line searches while achieving fast convergence for the box-constrained quadratic structure of the NCP. The solver iterates until convergence, yielding the optimal contact forces $\boldsymbol{\gamma}^*$, which are then used to compute $\mathbfcal{F}(\boldsymbol{\gamma}^*)$ and $\dot{\boldsymbol{\ell}}(\boldsymbol{\gamma}^*)$ for updating the colony state via \autoref{eq:colony_update}.
...
\begin{figure*}[p]
    ...
    \caption{Comparison of pattern formation between hard and soft collision models at similar colony sizes. Each row shows the colony evolutions up to a maximum colony radius of 100 (non-dimensionalized units, $\ell_0 = 1$). The color indicates the length of the cells (short cells are darker, longer cells are lighter). Clear concentric ring patterns are visible for both models at $\lambda = 10^{-3}$ confirming the results from~\cite{Weady2024}.}
...
\begin{figure}[H]
    ...
    \begin{subfigure}[b]{0.49\columnwidth}
        ...
        \caption{Soft model}
    \end{subfigure}
    \caption{Close-up comparison of cell packing in the colony center for both collision models at $\lambda = 10^{-3}$. The hard model (a) maintains near-optimal packing, while the soft model (b) exhibits significant overlap and unphysical crowding.}
...
\begin{figure}[H]
    ...
    \begin{subfigure}[b]{\linewidth}
        ...
        \caption{Radial packing fraction profiles ($\phi = \frac{A_{\text{cells}}}{A_{\text{total}}}$) for both models, taken at colony radius $R = 100$. The x-axis shows distance from the colony center (non-dimensionalized, $\ell_0 = 1$). Here $A_{\text{cells}}$ is the total area occupied by cells within a radial bin of width 2, and $A_{\text{total}}$ is the annular area of that bin (analogous to RDF binning). The hard model maintains nearly optimal packing, while the soft model exhibits higher fractions in the colony center, especially at low stress-sensitivity ($\lambda = 10^{-4}$).}
...
\begin{figure}[h]
    ...
    \caption{Total runtime to reach a colony radius of 100 versus CFL number on 112 cores. The x-axis shows the fixed CFL factor used across independent simulation runs, not the dynamic $\Delta t$ within a single run; each point corresponds to a separate simulation. The U-shaped curve reflects the cost of using a given CFL factor as a policy: small CFL requires many timesteps; large CFL inflates solver iterations per step. The red region indicates CFL values where the ReLCP procedure fails to converge.}
...
Future work could also investigate extensions to the current models to capture additional biological phenomena. External boundaries (e.g., confinement by a microfluidic channel) would allow study of stress feedback under geometric constraints relevant to biofilm formation. Nutrient diffusion fields would enable coupling of metabolic state to growth, capturing the starvation-driven growth arrest observed in large colonies~\cite{Warren2019}. Chemotactic signaling could reproduce directed collective migration seen in \textit{E. coli} swarms~\cite{You2018}. Most of these extensions can be integrated into the existing framework with minimal changes to the collision models~\cite{Yan_2020}.
