...
The distinction between hard and soft collision models lies solely in how the generalized force vector $\mathbfcal{F}$ and in particular the inter-cell forces $\mathbf{F}_{ij}$ are computed. The rest of the framework, including the colony representation, growth law, stress computation, and state integration, is identical for both models.
...
Both the smoothing factor $\alpha = 0.01$ and the 20\% cap were chosen empirically and were found to provide stable and robust timestep control across all tested colony sizes and stress-sensitivity regimes. In particular, these values effectively suppress oscillatory timestep updates while still allowing the method to adapt responsively to changes in colony dynamics.
