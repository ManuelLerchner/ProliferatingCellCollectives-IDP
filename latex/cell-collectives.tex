...
This severe overcrowding in the soft model does however not meaningfully impact the growth behavior and the expansion speed of the colony and does not affect the formation of concentric ring patterns (See \autoref{fig:sim_time_vs_colony_radius} and \autoref{fig:pattern_formation}). However, it does lead to significantly more particles being required to reach the same colony radius (See \autoref{fig:colony_radius_vs_num_cells}).  Especially for low stress-sensitivity ($\lambda \approx 0$) where cells tend to grow exponentially regardless of their expericenced stress, the soft model requires almost three times as many cells to reach the same colony radius as the hard model, indicating that the interior of the colony is severely overcrowded and unable to effectively contribute to the outward pressure of the colony.
...
The consistently higher stresses in the hard model are also present in the original work by Weady et al.~\cite{Weady2024} and are likely caused by the discrete cell-based model not fully matching the continuum assumptions made in the analytical derivation. The lower stresses in the soft model are likely caused by a missing global force propagation mechanism which limits the stress calculation to only consider local pairwise interactions based on the current overlap. This naturally causes the center of the colony to grow and divide unnaturally fast, causing severe overcrowding as described above in \autoref{sec:colony_growth_dynamics}. This effect can also be observed in \autoref{fig:radial_distribution_growth_rate}, which shows the radial cell growth rate for both models. Throughout the colony, cells in the soft model grow significantly faster than in the hard model, especially for low stress-sensitivity ($\lambda \approx 0$). It can also be observed that the under high stress sensitivity ($\lambda = 10^{-2}$), the growth rate is close to zero in throughout most of the colony for both models, except for a small region close to the edge. This means that at every point in time, only a thin layer of cells at the colony edge is actively growing and dividing, while the interior is effectively dormant. \autoref{fig:sim_time_vs_colony_radius} confirms that the colony deviates from exponential growth and instead expands linearly over time. This behavior is consistent with experimental observations of real \textit{E. coli} and \textit{Saccharomyces cerevisiae} colonies~\cite{Warren2019,Hallatschek2007,Giometto2018}.


\begin{figure}[h]
    \centering
    \includegraphics[width=\linewidth]{figures/comparison_plots/combined_radial_impedance.png}
    \caption{Radial relative growth rate profiles ($e^{-\lambda \sigma}$) for both models. For high stress-sensitivity, growth is effectively limited to a thin layer at the colony edge, while for low stress-sensitivity, cells grow exponentially throughout the colony.}
