...
\newcommand{\growthcomparisonrow}[6]{%
    ...
    \begin{subfigure}[b]{\subfigwidth}
        \includegraphics[width=\textwidth]{figures/growth_comparisons/#1_#2_#3.jpeg}
    \end{subfigure} &
    \begin{subfigure}[b]{\subfigwidth}
        \includegraphics[width=\textwidth]{figures/growth_comparisons/#1_#2_#4.jpeg}
    \end{subfigure} &
    \begin{subfigure}[b]{\subfigwidth}
        \includegraphics[width=\textwidth]{figures/growth_comparisons/#1_#2_#5.jpeg}
    \end{subfigure} &
    \begin{subfigure}[b]{\subfigwidth}
        \includegraphics[width=\textwidth]{figures/growth_comparisons/#1_#2_#6.jpeg}
    \end{subfigure}  \\
}
...
\begin{figure*}[p]
    ...
    %         \growthcomparisonrow{Hard}{e-1}{0090}{0100}{0110}{0120}
    %         \growthcomparisonrow{Soft}{e-1}{0140}{0170}{0240}{0310}
    %     \end{tabular}
    % \end{subfigure}
    ...
    \begin{subfigure}[b]{\textwidth}
        ...
        \begin{tabular}{r M{\subfigwidth} M{\subfigwidth} M{\subfigwidth} M{\subfigwidth}}
            \growthcomparisonrow{Hard}{e-2}{0070}{0080}{0090}{0099}
            \growthcomparisonrow{Soft}{e-2}{0080}{0090}{0110}{0160}
        \end{tabular}
    \end{subfigure}
    ...
    \begin{subfigure}[b]{\textwidth}
        ...
        \begin{tabular}{r M{\subfigwidth} M{\subfigwidth} M{\subfigwidth} M{\subfigwidth}}
            \growthcomparisonrow{Hard}{e-3}{0070}{0083}{0084}{0087}
            \growthcomparisonrow{Soft}{e-3}{0080}{0100}{0110}{0127}
        \end{tabular}
    \end{subfigure}
    ...
    \begin{subfigure}[b]{\textwidth}
        ...
        \begin{tabular}{r M{\subfigwidth} M{\subfigwidth} M{\subfigwidth} M{\subfigwidth}}
            \growthcomparisonrow{Hard}{e-4}{0070}{0080}{0084}{0087}
            \growthcomparisonrow{Soft}{e-4}{0070}{0091}{0100}{0120}
        \end{tabular}
    \end{subfigure}
...
TODO: mention benefints of adaptive timestep compared to hardcoded value of 1e-5
...
strong scaling1: How fast can we reach a colony radius of 100 with increasing number of processors?

strong scaling2: Disable division. initialize with a box of fully dense particles. Run for a fixed number of time steps (e.g. 1000). Measure wall-clock time T(P) to complete the simulation with P processors. Parallel efficiency E(P) = T(1) / (P * T(P)) if you also run a 1-core baseline for same total N.



weak scaling: How many updates per second per core can we achieve when increasing both the number of particles and processors proportionally?
% Turn off cell division (or immediately cap total particles so no birth happens).

% Initialize each processor with exactly N_local particles in its subdomain (same distribution they would normally have).

% Run for a fixed number of particle-updates per core (e.g. U = 1e7 updates/core).

% Measure:

% wall-clock time T(P) to complete U updates on each core,

% updates/sec/core = U / T(P),

% parallel efficiency E(P) = (T(1) / (P * T(P))) if you also run a 1-core baseline for same N_local.

% Expectation: updates/sec/core ~ constant and E(P) ≈ 1 if weak scaling is ideal.

% Pro: Clear, repeatable.
Con: Not representative of physics when division matters.
...
\begin{figure}[h]
    \centering
    \includegraphics[width=\linewidth]{figures/runtimes/strong_scaling_efficiency_hard_soft_logx.png}
    \caption{ }
\end{figure}
