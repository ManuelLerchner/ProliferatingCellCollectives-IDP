...
where $\mathbf{I}_3$ is the $3 \times 3$ identity matrix. Note that $\ell_i^k$ is the current length of cell $i$ at timestep $k$, not the reference length $\ell_0$; cells grow continuously so $\ell_i^k \neq \ell_0$ in general. The resulting matrix-vector product thus implements the overdamped Langevin equation from \autoref{eq:overdamped_langevin} for all cells simultaneously.
...
\begin{algorithm}[H]
    ...
    \begin{algorithmic}[1]
        ...
        \State Smooth the timestep change to avoid oscillations: $\Delta t_{\text{new}} = (1 - \alpha)\Delta t + \alpha \Delta t^*$ with $\alpha = 0.01$. \hfill\textit{(prevents oscillatory $\Delta t$ that could destabilise the solver)}
        \State Clamp $\Delta t_{\text{new}}$ within 20\% of current $\Delta t$ to prevent abrupt changes. \hfill\textit{(limits single-step acceleration to avoid overshoots)}
