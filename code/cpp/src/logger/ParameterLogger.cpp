...
void ParameterLogger::log(const SimulationParameters& params) {
  ...
  logger_.addFieldData("log_every_colony_radius_delta", params.sim_config.log_every_colony_radius_delta);
  logger_.addFieldData("log_every_sim_time_delta", params.sim_config.log_every_sim_time_delta);
