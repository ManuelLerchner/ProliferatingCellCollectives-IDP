...
void Domain::run() {
  ...
  for (int i = 0; i < num_steps; i++) {
    ...
    if (sim_config_.domain_resize_frequency > 0 && i > 0 && (i % sim_config_.domain_resize_frequency == 0)) {
      resizeDomain();
      exchangeParticles();
    }
...
void Domain::exchangeParticles() {
  ...
  // 4. Prepare receive buffer and displacements
  ...
  // 6. Update local particles
  // First, remove particles that have been sent to other ranks.
  auto& local_particles = particle_manager_->local_particles;
  local_particles.erase(std::remove_if(local_particles.begin(), local_particles.end(),
                                       [](const Particle& p) { return p.getToDelete(); }),
                        local_particles.end());

  // Add the newly received particles from other ranks.
  for (const auto& pd : recv_buffer) {
    local_particles.emplace_back(pd);
  }
...
void Domain::resizeDomain() {
  ...
  // 2. Allreduce to get global bounding box
  double global_min_x, global_max_x, global_min_y, global_max_y, global_min_z, global_max_z;
  MPI_Allreduce(&local_min_x, &global_min_x, 1, MPI_DOUBLE, MPI_MIN, PETSC_COMM_WORLD);
  MPI_Allreduce(&local_max_x, &global_max_x, 1, MPI_DOUBLE, MPI_MAX, PETSC_COMM_WORLD);
  MPI_Allreduce(&local_min_y, &global_min_y, 1, MPI_DOUBLE, MPI_MIN, PETSC_COMM_WORLD);
  MPI_Allreduce(&local_max_y, &global_max_y, 1, MPI_DOUBLE, MPI_MAX, PETSC_COMM_WORLD);
  MPI_Allreduce(&local_min_z, &global_min_z, 1, MPI_DOUBLE, MPI_MIN, PETSC_COMM_WORLD);
  MPI_Allreduce(&local_max_z, &global_max_z, 1, MPI_DOUBLE, MPI_MAX, PETSC_COMM_WORLD);

  // pad local mins and max to sim_config_.min_box_size
  if (global_max_x - global_min_x < size_ * sim_config_.min_box_size.x) {
    ...
    global_min_x = global_min_x - padding;
  ...
  if (global_max_y - global_min_y < size_ * sim_config_.min_box_size.y) {
    ...
    global_min_y = global_min_y - padding;
  ...
  if (global_max_z - global_min_z < size_ * sim_config_.min_box_size.z) {
    ...
    global_min_z = global_min_z - padding;
    global_max_z = global_max_z + padding;
  }
