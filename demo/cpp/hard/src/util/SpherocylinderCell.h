...
class DCPQuery {
 public:
  struct Result {
    Result()
        : distance(static_cast<T>(0)),
          ...
          closest{std::array<T, N>{}, std::array<T, N>{}} {
    }

    T distance, sqrDistance;
    std::array<T, 2> parameter;
    std::array<std::array<T, N>, 2> closest;
  };
  ...
  Result operator()(std::array<T, N> const& P0, std::array<T, N> const& P1,
  ...
  Result ComputeRobust(std::array<T, N> const& P0, std::array<T, N> const& P1,
                       std::array<T, N> const& Q0, std::array<T, N> const& Q1) {
    using namespace utils::ArrayMath;
    Result result{};
...
template <typename T, size_t N>
using DCPSegmentSegment = DCPQuery<T, N>;

template <typename T>
using DCPSegment2Segment2 = DCPSegmentSegment<T, 2>;

template <typename T>
using DCPSegment3Segment3 = DCPSegmentSegment<T, 3>;
}  // namespace utils::geometry
