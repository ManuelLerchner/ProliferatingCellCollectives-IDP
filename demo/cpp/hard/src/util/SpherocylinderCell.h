...
double DCPQuery::operator()(std::array<double, 3> const &P0, std::array<double, 3> const &P1,
  ...
  result.closest[0] = (1 - result.parameter[0]) * P0 + result.parameter[0] * P1;
  result.closest[1] = (1 - result.parameter[1]) * Q0 + result.parameter[1] * Q1;
...
inline double DCPQuery::GetClampedRoot(double slope, double h0, double h1) {
  ...
  if (fabs(slope - (h1 - h0)) >= 10 * eps * std::max(abs(h1), abs(h0))) {
    std::cout << "Inconsistent parameters in GetClampedRoot: "
              << "slope = " << slope << ", h1 = " << h1 << ", h0 = " << h0 << std::endl;
    return 0.5;
  }
