...
Constraint::Constraint(double delta0, bool violated, int gidI, int gidJ, int localI, int localJ, std::array<double, 3> normI, std::array<double, 3> posI, std::array<double, 3> posJ, std::array<double, 3> contactPoint, double stressI, double stressJ, int constraint_iterations, int gid)
    : delta0(delta0), violated(violated), gidI(gidI), gidJ(gidJ), localI(localI), localJ(localJ), normI(normI), rPosI(posI), rPosJ(posJ), contactPoint(contactPoint), stressI(stressI), stressJ(stressJ), constraint_iterations(constraint_iterations), gid(gid) {}
