...
PhysicsEngine::PhysicsMatrices PhysicsEngine::calculateMatrices(const std::vector<Particle>& local_particles, const std::vector<Constraint>& local_constraints) {
  ...
  MatWrapper S = calculate_stress_matrix(local_constraints, local_particles, mappings.configL2GMap, mappings.velocityL2GMap, mappings.lengthL2GMap, mappings.constraintL2GMap);
...
VecWrapper getLengthVector(const std::vector<Particle>& local_particles) {
  VecWrapper L;
  PetscCallAbort(PETSC_COMM_WORLD, VecCreate(PETSC_COMM_WORLD, L.get_ref()));
  PetscCallAbort(PETSC_COMM_WORLD, VecSetSizes(L, local_particles.size(), PETSC_DETERMINE));
  PetscCallAbort(PETSC_COMM_WORLD, VecSetType(L, VECSTANDARD));
  PetscCallAbort(PETSC_COMM_WORLD, VecSetFromOptions(L));
  ...
  PetscCallAbort(PETSC_COMM_WORLD, VecAssemblyBegin(L));
  PetscCallAbort(PETSC_COMM_WORLD, VecAssemblyEnd(L));
...
PhysicsEngine::SolverSolution PhysicsEngine::solveConstraintsRecursiveConstraints(ParticleManager& particle_manager, double dt) {
  ...
  while (constraint_iterations < solver_config.max_recursive_iterations && !converged) {
    ...
    if (constraint_iterations == 0) {
      ...
      MatCreateVecs(L_PREV.get(), NULL, t2_workspace.get_ref());
      MatCreateVecs(matrices.M.get(), NULL, t2_workspace.get_ref());
      // Allocate phi_dot_growth_result to match number of columns of L_PREV
      PetscInt ncols;
      MatGetSize(L_PREV.get(), NULL, &ncols);
      VecCreate(PETSC_COMM_WORLD, phi_dot_growth_result.get_ref());
      VecSetSizes(phi_dot_growth_result.get(), ncols, PETSC_DETERMINE);
      VecSetFromOptions(phi_dot_growth_result.get());
    } else {
      ...
      // Reallocate phi_dot_growth_result if number of columns of L_PREV changed
      PetscInt ncols, phi_dot_size;
      MatGetSize(L_PREV.get(), NULL, &ncols);
      VecGetSize(phi_dot_growth_result.get(), &phi_dot_size);
      if (phi_dot_size != ncols) {
        VecDestroy(phi_dot_growth_result.get_ref());
        VecCreate(PETSC_COMM_WORLD, phi_dot_growth_result.get_ref());
        VecSetSizes(phi_dot_growth_result.get(), ncols, PETSC_DETERMINE);
        VecSetFromOptions(phi_dot_growth_result.get());
    ...
    auto gradient = [&](const VecWrapper& gamma_curr, VecWrapper& phi_next_out) {
      ...
      // growth
      if (constraint_iterations == 0) {
        // no growth in first iteration
        VecZeroEntries(ldot_curr_workspace.get());
      } else {
      ...
      // phi_dot_movement = phi_dot_movement + phi_dot_growth
      PetscCallAbort(PETSC_COMM_WORLD, VecAXPY(phi_dot_movement_workspace.get(), 1.0, phi_dot_growth_result.get()));
    ...
    // calculate forces
    auto [f, u, dC] = calculate_forces(D_PREV, matrices.M, matrices.G, gamma_diff);
    ...
    particle_manager.moveLocalParticlesFromSolution({.dC = dC, .f = f, .u = u});
  ...
  particle_manager.growLocalParticlesFromSolution({.dL = ldot_prev, .impedance = impedance_curr_workspace});
