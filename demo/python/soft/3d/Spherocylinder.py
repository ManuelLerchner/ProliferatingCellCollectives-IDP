...
class Spherocylinder:
    ...
    def move(self, dt):
        """
        Move the particle using velocity Verlet integration (matching 2D model).
        """
        # Store current force and torque as old for next step
        self.old_force = self.force.copy()
        self.old_torque = self.torque.copy()
        ...
        self.apply_torque_verlet(dt)

        # Reset force and torque for next step
        self.force = np.zeros(3)
        self.torque = np.zeros(3)

    def apply_torque_verlet(self, dt):
        """
        Update angular velocity and orientation using velocity Verlet (matching 2D model, but for 3D/quaternions).
        """
        # Half-update angular velocity
        angular_acceleration = np.linalg.solve(
            self.inertia_tensor, self.old_torque)
        self.angular_velocity += 0.5 * angular_acceleration * dt
        ...
        if angle > 0:
            ...
            quat_rotation = np.array(
                [quat_rotation[3], quat_rotation[0], quat_rotation[1], quat_rotation[2]])
            new_orientation = self.quaternion_multiply(
                self.orientation, quat_rotation)
            self.orientation = new_orientation / \
                np.linalg.norm(new_orientation)

        # Second half-update angular velocity with new torque
        angular_acceleration_new = np.linalg.solve(
            self.inertia_tensor, self.torque)
        self.angular_velocity += 0.5 * angular_acceleration_new * dt
    ...
    def __repr__(self):
        return f"Spherocylinder(position={self.position}, length={self.length}, diameter={self.diameter})"
